<template>
    ...
    <div class="flex items-center py-3 h-screen flex-col">
        ...
        <div class="bg-white p-8 rounded shadow-md w-1/2">
            ...
            <h2 class="text-xl font-normal mb-4">Transfer Money</h2>
            <hr class="py-3" />
            ...
            <form @submit.prevent="depositMoney">
                <div class="mb-4">
                    ...
                    <input v-model="email" type="email" id="email" name="email"
                        class="mt-1 p-2 w-full border rounded-md" placeholder="Enter email address" />
    ...
    export default {
        ...
        setup() {
            ...
            const depositMoney = async () => {
                ...
                if(email.value==null){
                    email_error.value = 'Please enter valid email address';
                    return;
                }
                ...
                if(initiateTransactionResponse.value.data.status == 1){
                    ...
                    if(initiateTransactionResponse.value.data.email_exist){
                        email_error.value = initiateTransactionResponse.value.data.email_exist;
                        return;
                    }
                    else{
                        email_error.value = '';
            ...
            const processTransaction = async () => {
                ...
                if(initiateTransactionResponse.value.data.status == 1){
                    ...
                    if(processResponse.data.status == 1){
                        ...
                        if(processResponse.data.shortest_money){
                            transaction_otp_error.value = processResponse.data.shortest_money;
                        }
                        isLoading.value = false;
                        return;
                    }
